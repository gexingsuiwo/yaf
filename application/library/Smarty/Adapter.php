...
class Smarty_Adapter implements \Yaf\View_Interface
{
     ...
     * <a href="http://my.oschina.net/u/556800" class="referer" target="_blank">@return</a>  void
     */
    public function assign($tplVar, $value = null, $xssClean = 'ALL_HTML_TAGS') {
        $securityLib = new \Lib\Security();
        if ($xssClean == $securityLib::XSS_CLEAN_ALL_HTML_TAGS) {
            $value = $securityLib->dXssClean($value);
        } elseif ($xssClean == $securityLib::XSS_CLEAN_NOLY_SCRIPT_TAGS) {
            $value = $securityLib->dXssClean2($value);
        } elseif ($xssClean == $securityLib::XSS_CLEAN_NO_DENY) {
            $value = $value;
        } else {
            throw new \Exception('xss param Error');
        }
                
        if (is_array($tplVar)) {
            $this->_smarty->assign($tplVar);
            return;
        }
    ...
    public function display( $viewName, $tplVars = NULL ) {
        
         $view_path = $this->getScriptPath()[0] . '/' . $viewName . '.html';
         $cache_id     = empty($tplVars['cache_id']) ? '' : $tplVars['cache_id'];
         $compile_id = empty($tplVars['compile_id']) ? '' : $tplVars['compile_id'];
